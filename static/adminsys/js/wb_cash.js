...
function filter() {
    ...
    console.log(status);
    $.get("/operation/reward/get_one_to_one_cash_list/", {
        'status': status,
        'cellphone': cellphone,
...
function exchange_pass(type, record_id, user_id) {
    ...
    $.get('/operation/wb_operation_on_exchange_record/', {
            type: type,
            user_id: user_id,
            record_id: record_id,
            reason: '通过',
...
function exchange_delivery(type, record_id, user_id) {
    ...
    var pass_discount_id = "pass_discount_" + record_id;
    var onclick_des = "undoCash('" + status_id + "','" + after_undo_id + "'," + record_id + "," + user_id + ")";
    $.get('/operation/wb_operation_on_exchange_record/', {type: type, user_id: user_id, record_id: record_id},
        function (data) {
...
function batchDeliver() {
    ...
    $.get("/operation/reward/one_to_one_batch_teacher_deliver//", {
...
function displayTeacherCashList(data) {
    $(".exchange_list").html('');
    console.log(data);
    $.each(data.one_to_one_cash_info_list, function (key, value) {
        $(".exchange_list").append(
            "<tr>" +
            "<td>" + "<input name='deliver_record' type='checkbox'  class='all_post' value=' " + value.record_id + "'>" + "</td>" +
            "<td>" +
            "<p>" + "订单号: " + value.record_id + "</p>" +
            "<p>" + "申请兑换金额: " + value.cost_money + " 元" + "</p>" +
            "<p>" + "实际兑换金额: " + value.deal_money + " 元" + "</p>" +
            "</td>" +
            "<td>" + value.real_name + "</td>" +
            "<td>" + value.user_bank_account + "</td>" +
            "<td>" +
            "<p>" + "<a href = '/operation/post_num/?filter_type=5&user_id=" + value.user_id + "' >" + "用户id: " + value.user_id + "</a>" + "</p>" +
            "<p>" + "兑换（拒绝/总数）: " + value.failed_num + "/" + value.all_num + "</p>" +
            "</td>" +
            "<td>" + value.create_time + "</td>" +
            "<td>" + "<p>" + value.operation_time + "</p>" + "<p>" + "操作人员: " + value.operator_user_name + "</p>" + "</td>" +
...
$(function () {
    ...
    //加载信息
    $.get("/operation/reward/get_one_to_one_cash_list/", {}, displayTeacherCashList);
